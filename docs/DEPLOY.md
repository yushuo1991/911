# 🚀 部署和更新指南

本文档详细说明如何部署项目以及如何进行日常更新。

## 📋 目录

- [自动化部署系统](#自动化部署系统)
- [日常更新方法](#日常更新方法)
- [部署状态监控](#部署状态监控)
- [高级操作](#高级操作)
- [故障排查](#故障排查)

---

## 🎯 自动化部署系统

本项目已配置完整的 CI/CD 自动化部署系统：

### 工作流程

```
本地修改代码 → git push → GitHub Actions → 自动构建 → 自动部署到服务器 → 自动重启服务
```

### 部署配置

**GitHub 仓库：** https://github.com/yushuo1991/stock-tracker

**服务器信息：**
- IP: 107.173.154.147
- 用户: root
- 端口: 22
- 项目目录: `/www/wwwroot/stock-tracker` 或 `~/stock-tracker`

**GitHub Actions：**
- 配置文件: `.github/workflows/deploy.yml`
- 触发条件: 推送到 `main` 分支
- 部署时间: 约 3-5 分钟

---

## 🔄 日常更新方法

### 方法 1：Git 命令行（推荐）⭐

这是最简单、最常用的方法：

```bash
# 1. 查看修改的文件
git status

# 2. 添加所有修改
git add .

# 3. 提交修改（填写说明）
git commit -m "你的修改说明"

# 4. 推送到 GitHub（触发自动部署）
git push
```

**一键完成：**
```bash
git add . && git commit -m "update" && git push
```

**推送后会自动：**
1. ✅ 触发 GitHub Actions
2. ✅ 安装依赖 (`npm ci`)
3. ✅ 构建项目 (`npm run build`)
4. ✅ 通过 SSH 部署到服务器
5. ✅ 在服务器上拉取最新代码
6. ✅ 重新构建并重启 PM2 服务

**等待 3-5 分钟，你的更新就会生效！**

---

### 方法 2：GitHub 网页编辑

适合小修改（如更新文档、修改配置）：

1. 访问 https://github.com/yushuo1991/stock-tracker
2. 找到要修改的文件
3. 点击右上角的 ✏️ 编辑按钮
4. 修改内容
5. 页面底部填写提交说明
6. 点击 **"Commit changes"**
7. 自动部署！

---

### 方法 3：使用一键部署脚本

项目根目录提供了一键部署脚本：

**Windows：**
双击运行 `GitHub-CLI自动化.bat`

**或命令行：**
```powershell
powershell -ExecutionPolicy Bypass -File auto-deploy-github.ps1
```

脚本会自动：
- 检查 Git 状态
- 提交所有修改
- 推送到 GitHub
- 触发自动部署

---

## 📊 部署状态监控

### 在线查看（推荐）

访问 GitHub Actions 页面：
```
https://github.com/yushuo1991/stock-tracker/actions
```

你会看到：
- ✅ **绿色勾**：部署成功
- ❌ **红色叉**：部署失败
- 🟡 **黄色圆**：部署进行中

点击任意部署记录可查看详细日志。

---

### 命令行查看

**查看最近的部署记录：**
```bash
gh run list --repo yushuo1991/stock-tracker --limit 10
```

**实时监控当前部署：**
```bash
gh run watch --repo yushuo1991/stock-tracker
```

**查看失败的部署日志：**
```bash
gh run view --repo yushuo1991/stock-tracker --log-failed
```

---

## 🛠️ 高级操作

### 手动触发部署

即使没有代码修改，也可以手动触发部署：

**方法 1：GitHub 网页**
1. 访问 https://github.com/yushuo1991/stock-tracker/actions
2. 选择 "Auto Deploy to Server"
3. 点击 "Run workflow"
4. 选择分支（main）
5. 点击 "Run workflow"

**方法 2：命令行**
```bash
gh workflow run deploy.yml --repo yushuo1991/stock-tracker
```

---

### 回滚到之前的版本

如果新版本有问题，可以回滚：

```bash
# 1. 查看提交历史
git log --oneline

# 2. 回滚到指定版本（替换 <commit-hash>）
git reset --hard <commit-hash>

# 3. 强制推送（会触发重新部署）
git push --force
```

**⚠️ 注意：** 强制推送会覆盖远程历史，请谨慎使用！

---

### 修改部署配置

如果需要修改服务器信息、部署流程等：

**1. 修改服务器信息（GitHub Secrets）：**

访问：`https://github.com/yushuo1991/stock-tracker/settings/secrets/actions`

可修改的 Secrets：
- `SERVER_HOST` - 服务器 IP
- `SERVER_USER` - SSH 用户名
- `SERVER_PASSWORD` - SSH 密码
- `SERVER_PORT` - SSH 端口

**2. 修改部署流程：**

编辑文件：`.github/workflows/deploy.yml`

```yaml
# 例如：修改 Node.js 版本
- name: Setup Node.js
  uses: actions/setup-node@v4
  with:
    node-version: '20'  # 从 18 改为 20
```

修改后提交并推送即可生效。

---

### 服务器端操作

有时需要直接在服务器上操作：

**连接到服务器：**
```bash
ssh root@107.173.154.147
```

**进入项目目录：**
```bash
cd /www/wwwroot/stock-tracker
# 或
cd ~/stock-tracker
```

**手动重启服务：**
```bash
pm2 restart stock-tracker
```

**查看服务状态：**
```bash
pm2 status
pm2 logs stock-tracker
```

**手动拉取最新代码：**
```bash
git pull origin main
npm install
npm run build
pm2 restart stock-tracker
```

---

## 🐛 故障排查

### 部署失败常见问题

#### 1. 构建失败（Build project ❌）

**症状：** 在 "Build project" 步骤失败

**可能原因：**
- 代码语法错误
- TypeScript 类型错误
- 依赖版本冲突

**解决方法：**
```bash
# 本地先测试构建
npm run build

# 如果本地构建失败，修复错误后再推送
```

---

#### 2. SSH 连接失败（Deploy to Server via SSH ❌）

**症状：** 在 "Deploy to Server via SSH" 步骤失败

**可能原因：**
- 服务器密码错误
- 服务器 IP 变更
- 服务器 SSH 服务未启动

**解决方法：**
1. 检查 GitHub Secrets 中的服务器信息是否正确
2. 手动测试 SSH 连接：
   ```bash
   ssh root@107.173.154.147
   ```
3. 如果无法连接，联系服务器管理员

---

#### 3. 服务器上构建失败

**症状：** SSH 连接成功，但服务器上执行命令失败

**可能原因：**
- 服务器磁盘空间不足
- Node.js 版本不兼容
- PM2 未安装或配置错误

**解决方法：**
```bash
# 连接到服务器
ssh root@107.173.154.147

# 检查磁盘空间
df -h

# 检查 Node.js 版本
node -v  # 应该是 v18 或更高

# 检查 PM2
pm2 -v

# 手动运行部署脚本
cd /www/wwwroot/stock-tracker
git pull
npm install
npm run build
pm2 restart stock-tracker
```

---

#### 4. 部署成功但网站无法访问

**可能原因：**
- PM2 服务未启动
- 端口被占用
- 防火墙阻止访问

**解决方法：**
```bash
# 检查 PM2 状态
pm2 status

# 查看应用日志
pm2 logs stock-tracker --lines 100

# 重启应用
pm2 restart stock-tracker

# 查看端口占用
netstat -tuln | grep 3000
```

---

### 获取帮助

如果问题依然无法解决：

1. **查看完整日志：**
   - 访问 https://github.com/yushuo1991/stock-tracker/actions
   - 点击失败的部署记录
   - 查看每一步的详细输出

2. **检查服务器日志：**
   ```bash
   ssh root@107.173.154.147
   pm2 logs stock-tracker --lines 200
   ```

3. **创建 Issue：**
   - 访问 https://github.com/yushuo1991/stock-tracker/issues
   - 点击 "New issue"
   - 描述问题并附上错误日志

---

## 📝 部署检查清单

在推送重要更新之前，建议检查：

- [ ] 本地运行 `npm run build` 成功
- [ ] 本地运行 `npm start` 可以正常访问
- [ ] 已测试新功能
- [ ] 已更新相关文档
- [ ] commit message 清晰明了
- [ ] 确认要部署到生产环境

---

## 🎯 最佳实践

### 提交信息规范

使用清晰的 commit message：

```bash
# 功能开发
git commit -m "feat: 添加股票搜索功能"

# Bug 修复
git commit -m "fix: 修复交易日历时区问题"

# 文档更新
git commit -m "docs: 更新部署文档"

# 样式调整
git commit -m "style: 优化移动端布局"

# 性能优化
git commit -m "perf: 优化数据加载速度"

# 代码重构
git commit -m "refactor: 重构股票数据处理逻辑"
```

### 分支管理

```bash
# 开发新功能时创建分支
git checkout -b feature/new-chart

# 完成后合并到 main
git checkout main
git merge feature/new-chart
git push
```

### 定期备份

```bash
# 在服务器上定期备份数据
ssh root@107.173.154.147
cd /www/wwwroot/stock-tracker
tar -czf backup_$(date +%Y%m%d).tar.gz .
```

---

## 📞 联系支持

遇到问题？
- 📧 Email: your-email@example.com
- 💬 GitHub Issues: https://github.com/yushuo1991/stock-tracker/issues
- 📖 更多文档: [README.md](../README.md)

---

**⚡ 记住：更新代码只需三步！**
```bash
git add .
git commit -m "update"
git push
```




