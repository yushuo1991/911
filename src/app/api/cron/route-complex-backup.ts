    ...
    const response = await fetch('https://api.tushare.pro', {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          start_date: Math.min(...tradingDays.map(d => parseInt(d))).toString(),
          end_date: Math.max(...tradingDays.map(d => parseInt(d))).toString()
        },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validToken = 'Bearer cron-token-2025'; // 在生产环境中应该使用环境变量
    ...
    if (action === 'preload') {
      ...
      await logToDatabase('info', '定时任务开始执行', { date, action });
      await preloadStockData(date);
      ...
      for (let i = 0; i < 6; i++) {
        ...
        try {
          await preloadStockData(dateStr);
          results.push({ date: dateStr, success: true });
        } catch (error) {
          results.push({ date: dateStr, success: false, error: String(error) });
        }
